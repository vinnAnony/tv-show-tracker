            ...
            <div class="bg-white shadow w-full rounded-lg divide-y divide-gray-200">
                <div class="px-5 py-7">
                    <form @submit.prevent="register">
                        ...
                        <input v-model="signUpData.user_name" type="text" required class="border rounded-lg px-3 py-2 mt-1 mb-5 text-sm w-full" />
                        <label class="font-semibold text-sm text-gray-600 pb-1 block">Full Name</label>
                        <input v-model="signUpData.full_name" type="text" required class="border rounded-lg px-3 py-2 mt-1 mb-5 text-sm w-full" />
...
<script>
    ...
    export default {
        ...
        methods: {
            register(){
                axios.post("http://localhost:4400/api/auth/register",this.signUpData)
                    .then(response => {
                        Vue.swal({
                            toast: true,
                            position: 'top-end',
