<template>
    <div>
        <div class="w-full ">
            <div class="mx-2 pt-3">
                <form>
                    <div class="relative text-gray-300">
                        <div>
                            <input
                                    type="text"
                                    placeholder="Search any movies . . ."
                                    class="text-black w-full px-14 py-3 rounded-md bg-stone-900 border border-slate-300 focus:ring focus:ring-slate-400 focus:ring-slate-400/50 md:text-lg lg:w-3/6 lg:text-sm lg:px-10"
                                    v-model="searchKeyword"/>

                            <svg class=" w-5 h-5 absolute top-4 left-5 md:w-6 md:h-6 lg:w-5 lg:h-5 lg:top-3.5 lg:left-3 "
                                 viewBox="0 0 28 28"
                                 fill="none"
                                 xmlns="http://www.w3.org/2000/svg">
                                <g id="Iconly/Broken/Search">
                                    <g id="Search">
                                        <path
                                                id="Search_2"
                                                d="M21.7555 20.6065L18.3182 17.2458L18.2376 17.1233C18.0878 16.9742 17.883 16.8902 17.6692 16.8902C17.4554 16.8902 17.2505 16.9742 17.1007 17.1233C14.1795 19.8033 9.67815 19.949 6.58201 17.4637C3.48586 14.9784 2.75567 10.6334 4.87568 7.31017C6.9957 3.98697 11.3081 2.71685 14.9528 4.34214C18.5976 5.96743 20.4438 9.98379 19.267 13.7276C19.1823 13.9981 19.2515 14.2922 19.4487 14.4992C19.6459 14.7062 19.9411 14.7946 20.223 14.7311C20.505 14.6676 20.7309 14.4619 20.8156 14.1914C22.2224 9.74864 20.0977 4.96755 15.8161 2.94106C11.5345 0.914562 6.38084 2.25082 3.68905 6.08542C0.99727 9.92001 1.57518 15.1021 5.04893 18.2795C8.52268 21.4569 13.8498 21.6759 17.5841 18.7949L20.6277 21.7705C20.942 22.0765 21.4502 22.0765 21.7645 21.7705C22.0785 21.4602 22.0785 20.9606 21.7645 20.6503L21.7555 20.6065Z"
                                                fill="currentColor"/>
                                    </g>
                                </g>
                            </svg>
                        </div>

                    </div>
                </form>
            </div>
        </div>
        <div class="mt-7 w-full flex flex-wrap justify-center gap-2 md:gap-2 lg:gap-2 mb-4">
            <div class="w-4/5 my-2 md:w-1/4 lg:w-2/12"
                 v-for="movie in movies"
                 :key="movie.id">
                <movie :movie="movie"/>
            </div>
        </div>
        <vue-element-loading :active="false" :is-full-screen="true"/>
    </div>
</template>

<script>
    import MovieDetails from "./MovieDetails";
    import Movie from "../ui/Movie";
    import url from '../../api/index';
    import MovieSearchHome from "../ui/MovieSearchHome";
    import VueElementLoading from 'vue-element-loading'
    import {tvShowerAlert} from "../../api/alerts";
    import {debounce} from 'lodash';
    export default {
        name: "Home",
        components: {MovieSearchHome, Movie, MovieDetails,VueElementLoading},
        data(){
            return{
                movies: {},
                searchKeyword: "",
            }
        },
        beforeMount(){
            url
                .get("movies")
                .then((response)=>
                {
                    this.movies = response.data;
                })
        },
        methods: {
            searchMovie() {
                    url.get("movie-search",{
                        params:{
                            keyword:this.searchKeyword
                        }
                    })
                        .then(response => {
                            this.movies = response.data;
                            console.log(response.data)
                        })
                        .catch(error => {
                            let errors = error.response.data.errors;
                            for (const error of errors){
                                tvShowerAlert('error',error.msg);
                            }
                        });
            }
        },
        created() {
          this.debounceName = debounce(this.searchMovie,1000);
        },
        watch: {
            searchKeyword(){
                if (!this.searchKeyword) return;

                this.debounceName();
            }
        }
    }
</script>

<style scoped>

</style>